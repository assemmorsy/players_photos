<template>
  <div class="main" v-if="board && players">
    ...
    <div class="wrapper bottom-wrapper" v-if="board.team1.bottom">
      ...
      <preview id="bottom-image" class="image rounded-3" :style="`opacity:${parseInt(board.opacity) / 100}`" :width="268"
        :height="361" :image="players[board.team1.bottom].image" :coordinates="players[board.team1.bottom].coordinates" />
    </div>

    <div class="wrapper right-wrapper" v-if="board.team2.right">
      <img src="@/assets/images/RedFrame.svg" id="right-box" class="box" />
      <preview id="right-image" class="image rounded-3" :style="`opacity:${parseInt(board.opacity) / 100}`" :width="268"
        :height="361" :image="players[board.team2.right].image" :coordinates="players[board.team2.right].coordinates" />
    </div>

    <div class="wrapper left-wrapper" v-if="board.team2.left">
      <img src="@/assets/images/RedFrame.svg" id="left-box" class="box" />
      <preview id="left-image" class="image rounded-3" :style="`opacity:${parseInt(board.opacity) / 100}`" :width="268"
        :height="361" :image="players[board.team2.left].image" :coordinates="players[board.team2.left].coordinates" />
    </div>
  </div>
</template>
